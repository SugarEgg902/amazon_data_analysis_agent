...
本平台为公司 Amazon 跨境电商业务提供竞品数据采集、聚合与可视化分析能力，聚焦**储能电源赛道**。以 **OUKITEL** 为核心视角，持续监控 EcoFlow、Bluetti、Jackery、VTOMAN、Anker 五个主要竞品品牌在全球 10 个 Amazon 站点的表现，辅助产品、运营与管理层进行定价策略、选品决策和市场趋势研判。分析结果每日自动推送到钉钉群。

> **历史包袱**：本项目早期是**手机**赛道（竞品为 Blackview/Ulefone/CUBOT/DOOGEE/FOSSiBOT），后整体转向储能。`brand_models` / `daily_model_summary` 两张表，以及型号排名相关的 `aggregation/model_summary.py`、`pages/ModelRanking.tsx` 都是那个时期的遗留，**当前 0 行数据、功能未启用**。部分项目截图也还是手机时期的。
...
### 钉钉推送
- 每日 09:00 日报 / 周一 09:05 周报 / 每月 1 号 09:10 月报
- 内容：总览、6 品牌明细榜（OUKITEL 置顶）、环比、品类分布、异常提醒、排名异动、AI 分析、图表
- 详见下方 [钉钉推送配置](#钉钉推送配置)
...
```bash
# 配置文件不入库(含密钥),从模板复制后填写
cp config/config.example.py config/config.py
# 编辑 config/config.py,填入数据库密码、卖家精灵账号、钉钉凭据、OSS 密钥
```

> `config/config.py` 已在 `.gitignore` 中。**本仓库是公开的，任何密钥都不要提交**（`.pyc` 里也会带密钥明文，同样已忽略）。
...
run_llm_analysis(d, 'daily')     # 周报/月报: 'weekly' / 'monthly'
"
```
...
定时推送见 `backend/scheduler.py`，无需手动触发。
...
| **图片** | 只接受 URL，不支持 base64/附件。**拉图的不是查看者本人** —— 私有地址（`10.x`/`192.168.x`）谁都拉不到，必须公网托管，见下方 OSS |
...
钉钉消息里的图表**必须挂在公网可达的地址**。本机内网地址（`10.x`）无论如何都拉不到 —— 绑不绑 `0.0.0.0`、看的人在不在内网都没用，因为去抓图的是钉钉服务端，不是查看者的客户端。因此图表渲染后需上传至阿里云 OSS。

**未配置 OSS 时会自动跳过嵌图**（纯文字推送），而不是推一堆裂图。

### 一、创建 Bucket

任意地域，**保持私有**（默认 ACL，不要开公共读）—— 竞品营收数据不该裸奔在公网。目录无需手动创建，OSS 是扁平存储，`report-charts/` 只是 object key 前缀。
